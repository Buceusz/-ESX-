...
local function HarvestEkstaza(source)

	if CopsConnected < Config.RequiredCopsOpium then
		TriggerClientEvent('esx:showNotification', source, _U('act_imp_police', CopsConnected, Config.RequiredCopsEkstaza))
		return
	end
	...
	SetTimeout(5000, function()
		...
		if PlayersHarvestingOpium[source] == true then
			...
			local opium = xPlayer.getInventoryItem('Ekstaza')

			if opium.limit ~= -1 and opium.count >= opium.limit then
				TriggerClientEvent('esx:showNotification', source, _U('inv_full_ekstaza'))
			else
				xPlayer.addInventoryItem('ekstaza', 1)
				HarvestOpium(source)
			end
...
RegisterServerEvent('esx_drugs:startHarvestEkstaza')
AddEventHandler('esx_drugs:startHarvestEkstaza', function()

	local _source = source

	PlayersHarvestingEkstaza[_source] = true

	TriggerClientEvent('esx:showNotification', _source, _U('pickup_in_prog'))

	HarvestEkstaza(_source)

end)

RegisterServerEvent('esx_drugs:stopHarvestEkstaza')
AddEventHandler('esx_drugs:stopHarvestEkstaza', function()

	local _source = source

	PlayersHarvestingEkstaza[_source] = false

end)

local function TransformEkstaza(source)
	...
	SetTimeout(10000, function()
		...
		if PlayersTransformingOpium[source] == true then
			...
			local opiumQuantity = xPlayer.getInventoryItem('ekstaza').count
			local poochQuantity = xPlayer.getInventoryItem('ekstaza_pooch').count
			...
			elseif opiumQuantity < 5 then
				...
				xPlayer.removeInventoryItem('ekstaza', 5)
				xPlayer.addInventoryItem('Ekstaza_pooch', 1)
...
RegisterServerEvent('esx_drugs:startTransformEkstaza')
AddEventHandler('esx_drugs:startTransformEkstaza', function()

	local _source = source

	PlayersTransformingEkstaza[_source] = true
...
RegisterServerEvent('esx_drugs:stopTransformEkstaza')
AddEventHandler('esx_drugs:stopTransformEkstaza', function()

	local _source = source

	PlayersTransformingEkstaza[_source] = false

end)

local function SellEkstaza(source)

	if CopsConnected < Config.RequiredCopsEkstaza then
		TriggerClientEvent('esx:showNotification', source, _U('act_imp_police', CopsConnected, Config.RequiredCopsEkstaza))
		return
	end
	...
	SetTimeout(7500, function()
		...
		if PlayersSellingEkstaza[source] == true then

			local _source = source
  			local xPlayer = ESX.GetPlayerFromId(_source)

			local poochQuantity = xPlayer.getInventoryItem('Eekstaza_pooch').count

			if poochQuantity == 0 then
				TriggerClientEvent('esx:showNotification', source, _U('no_pouches_sale'))
			else
				xPlayer.removeInventoryItem('ekstaza_pooch', 1)
				if CopsConnected == 0 then
                    xPlayer.addAccountMoney('black_money', 300)
                    TriggerClientEvent('esx:showNotification', source, _U('sold_one_ekstaza'))
                elseif CopsConnected == 1 then
                    xPlayer.addAccountMoney('black_money', 500)
                    TriggerClientEvent('esx:showNotification', source, _U('sold_one_ekstaza'))
                elseif CopsConnected == 2 then
...
RegisterServerEvent('esx_drugs:startSellEkstaza')
AddEventHandler('esx_drugs:startSellEkstaza', function()

	local _source = source

	PlayersSellingEkstaza[_source] = true
...
RegisterServerEvent('esx_drugs:stopSellEkstaza')
AddEventHandler('esx_drugs:stopSellEkstaza', function()

	local _source = source

	PlayersSellingEkstaza[_source] = false
...
AddEventHandler('esx_drugs:GetUserInventory', function(currentZone)
    ...
    TriggerClientEvent('esx_drugs:ReturnInventory', 
		...
		xPlayer.getInventoryItem('ekstaza').count, 
		xPlayer.getInventoryItem('ekstaza_pooch').count,
		xPlayer.job.name, 
		currentZone
    )
end)
