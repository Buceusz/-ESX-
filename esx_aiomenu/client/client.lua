...
RegisterNUICallback('toggleVehicleLocks', function()
  ...
  if lastCar ~= nil then
    if lockStatus == 0 or lockStatus == 1 then
      ...
      TriggerEvent("chatMessage", "Info", {255, 255, 0}, "Drzwi ^1zamknięte^0.")
    elseif lockStatus == 2 then
      SetVehicleDoorsLocked(lastCar, 1)
      SetVehicleDoorsLockedForAllPlayers(vehicle, false)
      TriggerEvent("chatMessage", "Info", {255, 255, 0}, "Drzwi ^otwarte^0.")
    end
  else
    TriggerEvent("chatMessage", "Info", {255, 255, 0}, "Nie masz kluczy do tego pojazdu.")
  end
end)
...
AddEventHandler('sendProximityMessageID', function(id, message)
  ...
  if pid == myId then
    TriggerEvent('chatMessage', "[Dowód]" .. "", {0, 153, 204}, "^7 " .. message)
...
AddEventHandler('sendProximityMessagePhone', function(id, name, message)
  ...
  local pid = GetPlayerFromServerId(id)
  if pid == myId then
    TriggerEvent('chatMessage', "[Numer Telefonu]^3(" .. name .. ")", {0, 153, 204}, "^7 " .. message)
  elseif GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(myId)), GetEntityCoords(GetPlayerPed(pid)), true) < 19.999 then
    TriggerEvent('chatMessage', "[Numer Telefonu]^3(" .. name .. ")", {0, 153, 204}, "^7 " .. message)
  end
end)