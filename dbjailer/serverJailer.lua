	...
	local xPlayer 		= ESX.GetPlayerFromId(source)
		...
		if cm[1] == "/unjail" then
			if xPlayer.job.name == 'police' then
				...
				TriggerClientEvent('chatMessage', -1, 'SYSTEM', { 255, 0, 0 }, "Nie mozesz wżucić ludzi do więzienia")
			end
		elseif cm[1] == "/jail1" then
			if xPlayer.job.name == 'police' then
				...
				if GetPlayerName(tPID) ~= nil then
					...
					TriggerClientEvent('chatMessage', -1, 'SĘDZIA', { 0, 0, 0 }, GetPlayerName(tPID) ..' jest w więzieniu na  '.. (jT/60) ..'   Miesięcy')
					TriggerClientEvent('chatMessage', -1, 'SĘDZIA', { 255, 0, 0 }, GetPlayerName(tPID) ..' jest w więzieniu za  '.. reason ..'.')
