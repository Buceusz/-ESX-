		...
		if radar.shown  then
			if radar.freeze == false then
					...
					if IsEntityAVehicle(e) then
						...
						local fvspeed = GetEntitySpeed(e)*2.236936*1.609
						local fplate = GetVehicleNumberPlateText(e)
						radar.info = string.format("~y~Plate: ~w~%s  ~y~Model: ~w~%s  ~y~Speed: ~w~%s KMH", fplate, fmodel, math.ceil(fvspeed))
					end
					...
					if IsEntityAVehicle(j) then
						...
						local bvspeed = GetEntitySpeed(j)*2.236936*1.609
						local bplate = GetVehicleNumberPlateText(j)
						radar.info2 = string.format("~y~Plate: ~w~%s  ~y~Model: ~w~%s  ~y~Speed: ~w~%s KMH", bplate, bmodel, math.ceil(bvspeed))
