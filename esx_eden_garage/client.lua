...
function OpenMenuGarage(PointType)
	...
	if PointType == 'spawn' then
		table.insert(elements,{label = "Lista pojazdów", value = 'list_vehicles'})
	end

	if PointType == 'delete' then
		table.insert(elements,{label = "Dzierżawa pojazdów", value = 'stock_vehicle'})
	end

	if PointType == 'pound' then
		table.insert(elements,{label = "Odstaw pojazd ("..Config.Price.."$)", value = 'return_vehicle'})
	end
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		...
		function(data, menu)
			if(data.current.value.state)then
				...
				TriggerEvent('esx:showNotification', 'Twój pojazd jest na rozwidleniu')
			end
		end,
		function(data, menu)
			menu.close()
...
function reparation(prix,vehicle,vehicleProps)
	...
	local elements = {
		{label = "Wprowadź pojazd ("..prix.."$)", value = 'yes'},
		{label = "Odwiedź mechanika", value = 'no'},
	}
	ESX.UI.Menu.Open(
...
function ranger(vehicle,vehicleProps)
	...
	TriggerEvent('esx:showNotification', 'Twój pojazd jest w garażu')
end
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
			...
			if (valid) then
				...
				TriggerEvent('esx:showNotification', 'Nie możesz przechowywać tego pojazdu')
			end
		end,vehicleProps)
	else
		TriggerEvent('esx:showNotification', 'Nie ma pojazdu do wjazdu')
	end
...
AddEventHandler('eden_garage:hasEnteredMarker', function(zone)
	...
	if zone == 'spawn' then
		CurrentAction     = 'spawn'
		CurrentActionMsg  = "Naciśnij ~INPUT_PICKUP~ aby wybrać pojazd"
		CurrentActionData = {}
	end

	if zone == 'delete' then
		CurrentAction     = 'delete'
		CurrentActionMsg  = "Naciśnij ~INPUT_PICKUP~ aby odstawić pojazd"
		CurrentActionData = {}
	end
	
	if zone == 'pound' then
		CurrentAction     = 'pound_action_menu'
		CurrentActionMsg  = "Naciśnij ~INPUT_PICKUP~ aby odkupić pojazd"
		CurrentActionData = {}
	end
end)
		...
		function(data, menu)
			...
			ESX.TriggerServerCallback('eden_garage:checkMoney', function(hasEnoughMoney)
				if hasEnoughMoney then
					...
					ESX.ShowNotification('Nie masz wystarczająco dużo pieniędzy')						
				end
			end)
		end,
		function(data, menu)
